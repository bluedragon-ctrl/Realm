...
| Quest system — slice 1: foundation (`content/quests/*.json` loader; per-player `quests` block in saves; data-driven engine in `src/game/quests.js` subscribed to the event bus; objective types `kill`/`room_clear`/`pickup_item`/`enter_room`/`deliver_item`; auto-discovery via `enter_room`/`kill`/`pickup_item`; `quests` / `quests <id>` command for active+completed list with progress; toasts on objective advance and completion; rewards block grants XP, gold, items via existing helpers; `deliver_item` consumes the given instance via `tryQuestDelivery` ahead of NPC exchanges so quest items are never swallowed by a matching sink/buy entry; any friendly NPC can be a recipient with no NPC def change required; exchanges may carry an optional `requires: { quest, status?, objective? }` block — locked exchanges are hidden from inspect / chip / buy / sell / give resolution per-player, and a `quest.unlock_offers` toast names the NPC when a quest/objective completion opens new wares; first three content quests — `quest.miner_rescue`, `quest.matriarch_hunt`, `quest.bat_daemon` — anchor the engine on the existing trapped-miner / matriarch / bat-daemon content) |

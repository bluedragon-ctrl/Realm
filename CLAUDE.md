...
- **Spells:** content in `content/spells/<id>.json`. Same verb-shape as socials/use. Players have `knownSpells: [id, ...]` — new players start with none and learn spells via loot/teaching. Admins are auto-granted `spell.heal` + `spell.spark` on login for testing (see `ADMIN_GRANTED_SPELLS` in `src/game/actors.js`). `cast` deducts MP, broadcasts via `runVerb`, then applies `spell.effect`.

...
- **Spells:** content in `content/spells/<id>.json`. Same verb-shape as socials/use. Players have `knownSpells: [id, ...]` — new players start with none and learn spells via loot/teaching. Admins are auto-granted every spell in `world.spellDefs` on login for testing (see `makePlayerActor` in `src/game/actors.js`). `cast` deducts MP, broadcasts via `runVerb`, then applies `spell.effect`.
...
- **Combat UX (client):** all NPC chips open the same popover (`Look` + `Give ▶` + socials) regardless of disposition. Hostile NPC chips keep the red `.hostile` CSS class as a danger signal but do not fire `attack` on click — attacking is typed (`attack <name>`) or driven by spell chips. Spell chips drive their own targeting per `target` metadata: `self` → cast immediately; `hostile` → if 1 hostile in room fire immediately, if multiple show a mini-popover of hostile chips, if 0 send the cast and let server return `cast.needs_hostile`; `friendly`/`any` → popover with `Yourself` + room targets (hostiles excluded for `friendly`).
...
- **Naming (content IDs and filenames):** see the `realm-content` skill (Step 2) for the full id/folder scheme. Filename always equals the id plus `.json`. Lowercase, snake_case, ASCII.
